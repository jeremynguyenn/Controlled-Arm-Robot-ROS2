...
private:
    ...
    void execute(const std::shared_ptr<GoalHandle> goal_handle)
    {
        ...
        if(success) {
            ...
            RCLCPP_INFO(this->get_logger(), "%s - MoveXYZW: Planning successful!", my_param.c_str());
            ...
            if (goal_handle->is_canceling()) {
                ...
                RCLCPP_INFO(this->get_logger(), "%s - MoveXYZW: Movement executed!", my_param.c_str());
                result->result = "MoveXYZW:SUCCESS";
                goal_handle->succeed(result);
            }

        } else {
            RCLCPP_INFO(this->get_logger(), "%s - MoveXYZW: Planning failed!", my_param.c_str());
