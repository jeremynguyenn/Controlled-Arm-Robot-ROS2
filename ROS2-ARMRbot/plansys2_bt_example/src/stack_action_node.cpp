...
public:
  StackAction()
  : plansys2::ActionExecutorClient("stack", 250ms)
  {
    progress_ = 0.0;
  }
 ...
 std::vector<std::string> something = current_arguments_;
  ...
  bool callServer(const std::vector<std::string>& args)
  {
   ...
   auto result2 = result1.get();
    ...
    auto client = node->create_client<tu_int::srv::ExecuteActionsd>("execute_actionsd");
    ...
    // Create the request
    auto request = std::make_shared<tu_int::srv::ExecuteActionsd::Request>();
    request->posit.x = randx;
    request->posit.y = randy;
    request->posit.z = randz;
    request->id = args[1];
    request->pos = args[3];
    RCLCPP_INFO(node->get_logger(), "stack---------------------%s", request->id.c_str());
    RCLCPP_INFO(node->get_logger(), "stack---------------------%s", request->pos.c_str());
    RCLCPP_INFO(node->get_logger(), "stack---------------------%f", randx);
    RCLCPP_INFO(node->get_logger(), "stack---------------------%f", randy);
